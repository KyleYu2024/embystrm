...
namespace StrmAssistant.Common
{
    public class ChapterApi
    {
        ...
        private List<ChapterInfo> GetChapters(BaseItem item, params MarkerType[] markerTypes)
        {
            var markerSet = new HashSet<MarkerType>(markerTypes);

            return _itemRepository.GetChapters(item)
                .Where(c => markerSet.Contains(c.MarkerType))
                .ToList();
        }

        public bool HasIntro(BaseItem item)
        {
            return GetChapters(item, MarkerType.IntroStart).Any();
        }

        public long? GetIntroStart(BaseItem item)
        {
            var introStart = GetChapters(item, MarkerType.IntroStart).FirstOrDefault();
        ...
        {
            var introEnd = GetChapters(item, MarkerType.IntroEnd).FirstOrDefault();
        ...
        {
            return GetChapters(item, MarkerType.CreditsStart).Any();
        }

        public long? GetCreditsStart(BaseItem item)
        {
            var creditsStart = GetChapters(item, MarkerType.CreditsStart).FirstOrDefault();
